...
const SignIn = ({ setToken }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Store the JWT token returned from the backend (usually in localStorage or sessionStorage)
      localStorage.setItem('token', response.data.token); // Store token for future use
      setToken(response.data.token); // Set the token in state to manage authentication
